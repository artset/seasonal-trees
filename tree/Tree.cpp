...
//    string = "F++F++F";
    ...
    for (size_t i = 0 ; i < string.size() ; i++) {
        switch (string[i]) {
            ...
            case 'X': {
                if (translateCoeff != 0) {
                    glm::vec3 rotatedTranslate = (currState.rotate * glm::vec4(TRANSLATE, 1.f)).xyz();
                    currState.transform = glm::translate(glm::mat4(), rotatedTranslate) * currState.transform;
                }
                mats.push_back(currState.transform);
                break;
            }
            case '-': {
                glm::vec3 axis = ROTATE_AXES[0];
                glm::mat4 rot = glm::rotate(-Tree::ANGLE, axis);
                currState.rotate = rot * currState.rotate;
                currState.transform = rot * currState.transform;
                translateCoeff = -1.f;
                break;
            }
            case '+': {
